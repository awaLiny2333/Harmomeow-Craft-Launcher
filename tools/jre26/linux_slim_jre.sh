#!/bin/sh
# 容器内：用官方 JDK26 的 jlink，按「原 HOML JRE 的模块集」裁出精简 lib/modules（jimage）。
# 用法（容器内）:
#   sh /mnt/linux_share/Documents/Meow/Codes/HMOS/HarmonyOS_Projects/HarmonyOS_Projects/Meowcraft/Harmomeow-Craft-Launcher/tools/jre26/linux_slim_jre.sh
set -u
...
BOOT="$WORK/bootjdk"
[ -x "$BOOT/bin/jlink" ] || BOOT="$J26/linux-bootjdk"
[ -x "$BOOT/bin/jlink" ] || { echo "找不到 boot JDK(jlink): $BOOT"; exit 2; }
echo "boot JDK: $("$BOOT/bin/java" -version 2>&1 | head -1)"

# 模块集 = 原 HOML JRE 的 MODULES（已知 MC 1.6.x–26.3 可用）；两处 26 版适配：
#   - **已剔除 jdk.crypto.cryptoki**（其 native libj2pkcs11 已删，保留模块会不一致）
#   - **已剔除 jdk.jsobject**（JDK26 已移除该模块，保留 jlink 会报错）
...
jdk.httpserver jdk.internal.vm.ci jdk.jdwp.agent jdk.jfr jdk.localedata jdk.management \
...
cp -f "$OUT/lib/modules" "$J26/out-linux/modules.slim"
cp -f "$OUT/release" "$J26/out-linux/release.slim"
echo "  -> $J26/out-linux/modules.slim ($(stat -c %s "$J26/out-linux/modules.slim") bytes)"
sha256sum "$J26/out-linux/modules.slim" | sed 's/^/  sha256 /'
echo "（把两行 bytes 贴回即可。）"
