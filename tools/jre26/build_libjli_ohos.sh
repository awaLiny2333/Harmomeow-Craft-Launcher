...
# 源：OpenJDK 26 源树（已含 patches/libjli-ohos.patch 的 OHOS 分体改动）。
# 用法:
...
# classfile_constants.h 由模板生成（替换类文件版本占位符）；JDK26 major=70, minor=0
GEN="$(cd "$(dirname "$OUT")" && pwd)/gen"
mkdir -p "$GEN"
sed -e 's/@@VERSION_CLASSFILE_MAJOR@@/70/' -e 's/@@VERSION_CLASSFILE_MINOR@@/0/' \
...
-DJDK_MAJOR_VERSION=26 -DJDK_MINOR_VERSION=0 -DJDK_MICRO_VERSION=0 -DJDK_UPDATE_VERSION=0"
