...
| OpenJDK 源码 | https://github.com/openjdk/jdk | tag **`jdk-26-ga`** → commit `4408cd2a07a14243a58cd9d30813302bfbe81133`（自编 `libjli`+`libjvm` 同用） |

- 官方 tar 同时用作**容器里的 boot JDK**（glibc/aarch64，openEuler 可直接跑）。
- ⚠️ **`jdk-26-ga` 是注解 tag**：`tag` 对象哈希是 **`04f966ef4e3`**（脚本早期误打此值），**引用 commit 必须用**
  `git rev-parse 'jdk-26-ga^{commit}'` = `4408cd2a07a14243a58cd9d30813302bfbe81133`。
- 官方包 `release` 里的 `SOURCE=".:git:d55edf1cba61"`（26.0.2.1 那支）**不是 tag**；如需与官方数据精确对齐
  可用该 commit 重编（`export JDK_TAG=<commit>`，容器需联网 fetch）。
- **共享源码 clone**：本工序复用 `stuffs/research/jdk25/jdk`（自包含 clone），经符号链接 `stuffs/research/jdk26/jdk -> ../jdk25/jdk` 暴露
  （容器挂载内同样可解析；worktree `jdk26src` 因 `.git` 指向绝对路径，**不能**在容器里用）。
...
| `libjvm.so`（容器编） | ✅ **逐字节** | `SOURCE_DATE_EPOCH=1770909215` + 固定 `$WORK=$HOME/meow-jvm26`；`linux_verify_jvm_repro.sh` **3× cmp 一致**（未魔改 glibc 件 sha `faa773b9…`） |

- **路径必须规范（前提）**：产物会嵌入**源码路径**（`libjli` 内嵌 `.../jdk26src/src/...`）与部分 native 的**输出路径**（写进 `.dynstr`）
...
| `patches/libjli-ohos.patch` | libjli OHOS 分体 patch（见 §3）——**对 jdk-26-ga 源干净可打** |
...
#    OpenJDK 源码：复用 stuffs/research/jdk25/jdk（含 jdk-26-ga tag）
tar xzf stuffs/research/jdk26/openjdk-26.0.2.1_linux-aarch64_bin.tar.gz -C stuffs/research/jdk26/_inspect
ln -sfn ../jdk25/jdk stuffs/research/jdk26/jdk
git -C stuffs/research/jdk25/jdk worktree add stuffs/research/jdk26/jdk26src jdk-26-ga

# 1) 容器（openEuler 24.03 aarch64；环境见「构建环境」）—— 编 libjvm + 瘦 modules
sh tools/jre26/linux_bootstrap.sh         # 装工具链 + boot JDK（如已装可跳过）
sh tools/jre26/linux_build_jvm.sh         # → out-linux/libjvm.so（glibc；SOURCE_DATE_EPOCH 固定）
sh tools/jre26/linux_verify_jvm_repro.sh  # （建议）3× cmp 验 libjvm 逐字节可复现
sh tools/jre26/linux_slim_jre.sh          # → out-linux/modules.slim（jlink）
...
# 4) 部署：先 HSP 后 HAP；改 JRE 后须卸载/清数据让 meow_jre.tar.gz 重新解压
```
...
| 自编 glibc `libjvm.so`（未魔改，27,706,232 B） | `faa773b9e961cd8da7f152bf417148a6c273c76f1ea3b0e796c13b426379a7d9` |
| 随包 `libjvm.so`（魔改后） | `3a18236d3edc165a12ae9866558d216dda555654e4459daa9652ae1b8104e0e6` |
| 随包 `libc6.so` | `c8b06fadd6f074b8bab3069bddb537bf503ac815575b46021b6ef05121a5a723` |
| 随包 `libjli.so` | `cc9d47e11032b2728ab016eca3bde1f13f24d06eacc5255814400d241f7116f0` |
...
| HAP（本次 debug 签名） | `256a9c396a0476fe5f4bbd5148d489120c69ae07abfeef39c68eaa6ea3098005` |
| HSP（本次 debug 签名） | `e83cf1089bcb2e4708be79786ced19ba2bd223bd75c2f113bb86c926be6bc1b3` |
...
> **JDK26 无需改 `glibc_compat.c`**：`verify_symbols.py` 对 jdk26 全部 29 件 **PASS**。
...
从 JDK26 源独立编（9 个 C 文件，链接 `-lz`；生成 `classfile_constants.h`，**major=70**）。
OHOS 分体 patch（3 处）：
...
2. `linux_build_jvm.sh`：取干净源（挂载 clone 的 `jdk-26-ga`）→ 打 `patches/os_linux-ohos.patch` → `configure --with-stdc++lib=static` → `make hotspot` → 拷 `libjvm.so` 回共享目录。
...
- **数据门（防混合）**：`Install` 解压后写 `<installDir>/meow_jre_data`（令牌 `26.0.2.1+1-7-r1`）；ArkTS `Paths.JRE_DATA_TOKEN`/`JavaEnvScanner` 同步校验；令牌不符 → 未就绪 + `Install` 清目录重解压。**⚠️ 改随包 JRE 数据的任何内容（模块集/文件）都必须同步升令牌**（native `kJreDataToken` + ArkTS `JRE_DATA_TOKEN`）。
...
| 自编 `libjli.so` | NEEDED=`libz.so`+`libc.so`；编译自 jdk-26-ga 源 |
| 自编 `libjvm.so`（容器编，glibc） | 27,706,232 B；魔改后 NEEDED=`libc.so`+`libc6.so`×2；符号满足性 PASS |
...
- **OHOS 分体布局**：`.so` 必须在 **el1**（随包）；数据区（el2）`dlopen` 一律失败。
- **`jdk-26-ga` 是注解 tag**：打印 commit 用 `^{commit}`（否则得到 tag 对象哈希）。
- **worktree 不能进容器**：`jdk26src` 的 `.git` 指向宿主绝对路径 → 容器里 `git` 失效；容器只用自包含主 clone（`jdk26/jdk` 符号链接）。
...
> **容器脚本调用约定**：所有 `linux_*.sh` 只在**容器内**跑；默认挂载根 `MEOW_SHARE=/mnt/linux_share/Documents/Meow/…/Meowcraft`、构建目录 `MEOW_WORK=$HOME/meow-jvm26`，可用环境变量覆盖。**可复现要求固定绝对路径**：`libjli` 的 `--src`（默认 `…/stuffs/research/jdk26/jdk26src`）、`libjvm` 的 `$WORK` 与源 tag（默认 `jdk-26-ga`）。
...
| 2 | 源码 tag `jdk-25-ga` → **`jdk-26-ga`**；`SOURCE_DATE_EPOCH` `1755018936` → **`1770909215`** | `linux_build_jvm.sh`、`linux_slim_jre.sh`、`linux_verify_jvm_repro.sh` |
...
- 可选精修：与官方 **26.0.2.1** 精确对齐（`SOURCE=git:d55edf1cba61`），需容器联网 fetch。
