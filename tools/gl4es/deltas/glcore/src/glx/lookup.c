...
/* Meowcraft fork: GL3 core backend hook */
#include "../gl/meowcore/meowcore.h"
...
void *gl4es_glXGetProcAddress(const char *name) {
    /* Meowcraft: in GL3/core mode the lookup is served *entirely* by the native GLES 3.2
     * backend (meowcore_*): its own wrapper, the real GLES symbol, or a safe no-op for
     * names this device lacks. It never returns NULL (a NULL would let LWJGL fall back to
     * dlsym on libgl4es.so and run gl4es' uninitialized FPE code -- measured crash
     * libgl4es.so(glClearDepthf+48) @0x48). We never fall through to the FPE here. */
    if (meowcore_active()) {
        return meowcore_GetProcAddress(name);
    }
