...
- **带 `--delta` 的配方逐字节可复现**：`deltas/glcore` 覆盖 `build_info.c`（去掉时间戳）并加入 **GL3 核心后端**；另加 `-DCMAKE_SKIP_RPATH=ON`（否则产物会带一条指向**临时副本目录**的 `RUNPATH` —— 开发路径泄漏）。已验证 2× 干净重建 `cmp` 相等（digest `b49407dff4f2988c0f0ae9409b496def13d0a99656d6da3c6578cd3de0bea319`，1,247,512 B）——**digest 随 delta 增长而变，换装时同步更新**（随包件 `libs/meowlwjgls/libs/natives.manifest` 同步）。
