#!/bin/sh
# 容器内 bootstrap：装原生工具链 + 用宿主自带的官方 JDK25(glibc/aarch64) 当 boot JDK 验证。
# 用法（容器内）:
#   sh /mnt/linux_share/Documents/Meow/Codes/HMOS/HarmonyOS_Projects/HarmonyOS_Projects/Meowcraft/Harmomeow-Craft-Launcher/tools/jre25/linux_bootstrap.sh
set -u
...
echo; echo "========== 3. 解官方 JDK25(glibc/aarch64) 当 boot JDK =========="
EXP_SHA=671208d205e70c9805da45a483f670d49dd64654990a7b7223ccffb2abb070dd
TAR="$J25/openjdk-25.0.2_linux-aarch64_bin.tar.gz"
...
BOOT="$J25/linux-bootjdk"
if [ -x "$BOOT/bin/java" ]; then
    echo "  已存在: $BOOT"
else
    mkdir -p "$BOOT"
    tar xzf "$J25/openjdk-25.0.2_linux-aarch64_bin.tar.gz" -C "$BOOT" --strip-components=1
fi
...
echo; echo "========== 4. JDK25 源可用性（用挂载的 clone 的 tag，离线） =========="
SRC_REPO="$J25/jdk"
if [ -d "$SRC_REPO/.git" ] || [ -f "$SRC_REPO/.git" ]; then
    echo "  clone 可见: $SRC_REPO"
    git -C "$SRC_REPO" tag --list "jdk-25*" 2>/dev/null | tail -5 | sed 's/^/    /'
else
