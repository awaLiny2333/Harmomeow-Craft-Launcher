#!/bin/sh
# 把「官方 OpenJDK(glibc) + JDK25 源 + 自编 glibc libjvm」组装成随包 JRE 集（libs + 数据）。
#
# 产物（<out>/）：
...
#
# 用法:
#   rebuild_for_meowcraft.sh --official-jdk <官方JDK目录> --jdk-src <JDK25源树> \
...
完成 -> $OUT
随包步骤（工程内）：
  1) 用 $OUT/*.so 替换 libs/meowjre25/libs/arm64-v8a/ 下同名件
  2) 用 pack_jre_data.py 规范化打包 $OUT/home 为 entry/src/main/resources/rawfile/meow_jre25.tar.gz
     （python3 $HERE/pack_jre_data.py --home $OUT/home --out <rawfile>/meow_jre25.tar.gz）
  3) rm -rf libs/meowjre25/build entry/build && devecocli build --modules entry meowjre25
