...
public class TinyFileDialogs {
    ...
    // --- [ Meowcraft 3.4.x compat shim: tinyfd_messageBox boolean overload ] ---
    //
    // LWJGL 3.4.x changed the generated mapping of tinyfd_messageBox's last C parameter
    // (int aDefaultButton): 3.3.x exposed it as boolean and returned boolean, 3.4.x
    // exposes it as int and returns int. Minecraft <= 1.21.11 links the OLD boolean
    // form, so re-add it, delegating to the int form.

    /** Displays a message dialog (stb/tinyfd 3.3.x boolean form). */
