...
public class STBImageResize {
    ...
    // --- [ Meowcraft 3.4.x compat shim: stb_image_resize v1 -> resize2 ] ---
    //
    // LWJGL 3.4.x moved to stb_image_resize2, which renamed the simple entry points:
    // the v1 public stbir_resize_uint8 / native nstbir_resize_uint8 (returning int) is
    // gone, replaced by stbir_resize_uint8_linear / nstbir_resize_uint8_linear
    // (returning a pointer). Minecraft <= 1.21.11 still links the OLD
    // nstbir_resize_uint8(JIIIJIIII)I (NativeImage resize), so re-express it on top of
    // the new linear entry point.
    //
    // v1 built the simple entry point as: linear colorspace, default filters,
    // STBIR_ALPHA_CHANNEL_NONE -> the resize2 layout with NO alpha handling.

    /** Maps the stb_image_resize v1 {@code num_channels} arg to a stb_image_resize2 pixel layout. */
    private static int meowV1Layout(int num_channels) {
        switch (num_channels) {
    ...
    /** Unsafe version of: {@link #stbir_resize_uint8 resize_uint8} (stb_image_resize v1). */
    public static int nstbir_resize_uint8(long input_pixels, int input_w, int input_h, int input_stride_in_bytes, long output_pixels, int output_w, int output_h, int output_stride_in_bytes, int num_channels) {
        return nstbir_resize_uint8_linear(input_pixels, input_w, input_h, input_stride_in_bytes, output_pixels, output_w, output_h, output_stride_in_bytes, meowV1Layout(num_channels)) != 0L ? 1 : 0;
    }

    /** {@code int stbir_resize_uint8(unsigned char const * input_pixels, int input_w, int input_h, int input_stride_in_bytes, unsigned char * output_pixels, int output_w, int output_h, int output_stride_in_bytes, int num_channels)} (stb_image_resize v1). */
    public static boolean stbir_resize_uint8(@NativeType("unsigned char const *") ByteBuffer input_pixels, int input_w, int input_h, int input_stride_in_bytes, @NativeType("unsigned char *") ByteBuffer output_pixels, int output_w, int output_h, int output_stride_in_bytes, int num_channels) {
        return stbir_resize_uint8_linear(input_pixels, input_w, input_h, input_stride_in_bytes, output_pixels, output_w, output_h, output_stride_in_bytes, meowV1Layout(num_channels)) != null;
    }
