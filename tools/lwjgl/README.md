# tools/lwjgl — self-build the LWJGL jars + natives for HarmonyOS (two generations)

Builds, from source, every LWJGL artifact we ship: a fat `lwjgl-<ver>.jar` per
generation, the per-generation natives, the `libffi.a` the 3.4.x core links, and
the multi-generation `meowcraft_extras.tar.gz` bundle. Everything is path-parameterized
and reproducible (same tag + SDK + build path → byte-identical; see digests below).
...
| `liblwjgl_333.so` / `_opengl_333` / `_stb_333` (3.3.3) | `libs/meowlwjgl3/libs/arm64-v8a/` | `rebuild_for_meowcraft.sh` → `install_natives.sh` |
...
| **per-generation** | `deltas/overlay-3.3.3/`, `deltas/overlay-3.4.3/` | 2 each | `opengl/GLCapabilities.java`, `glfw/GLFW.java` | **version-pinned** — see below |
...
## 2. Build each generation

```sh
SDK=${OHOS_SDK_NATIVE:-$HOME/devecow/deveco_tools/sdk/default/openharmony}

# --- 3.3.3 (MC 1.17–1.21.x) ---
sh tools/lwjgl/build_lwjgl_jar.sh --version 3.3.3 \
    --overlay tools/lwjgl/deltas/overlay --overlay tools/lwjgl/deltas/overlay-3.3.3
    # javac must be run by YOU -> stuffs/research/lwjgl_build-3.3.3/out/lwjgl.jar
sh tools/lwjgl/rebuild_for_meowcraft.sh 3.3.3        # build + install -> liblwjgl*_333.so (+ manifest)
...
- `tools/lwjgl/install_natives.sh` owns this directory: it copies a generation's build
  output in under the suffix and records `libs/meowlwjgl3/libs/natives.manifest`
...
python3 tools/lwjgl/pack_extras.py \
  ...
  --jar lwjgl-3.3.3.jar=stuffs/research/lwjgl_build-3.3.3/out/lwjgl.jar \
  --jar lwjgl-3.4.3.jar=stuffs/research/lwjgl_build-3.4.3/out/lwjgl.jar \
  --require lwjgl-3.3.3.jar --require lwjgl-3.4.3.jar \
...
- **Native builds are byte-reproducible only when the absolute `--src`/`--out` paths are
  ...
  `libs/meowlwjgl3/libs/arm64-v8a`); a rebuild at a different path is functionally identical
...
- After changing `libs/meowlwjgl3/libs/arm64-v8a/`, **clean the module build** before
